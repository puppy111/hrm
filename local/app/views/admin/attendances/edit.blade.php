...
@section('mainarea')
			...
			<div class="row">
				...
				<form action="#" id="form_bulk" class="form-horizontal" method="post" enctype="multipart/form-data">
				<input type="file" name="import_file" class="form-control">
				<button class="btn btn-primary pull-left" id="btnSave" onclick="save()">Import File</button>
				</form>
                                        ...
                                        <div class="form-actions">
                                            <div class="row">
                                                <div class="col-md-offset-3 col-md-9">
                                                    <button type="submit" data-loading-text="Submitting..." class="demo-loading-btn btn green"><i class="fa fa-edit"></i> Submit</button>
...
function save()
{
	...
	var formData = new FormData($('#form_bulk')[0]);
	//alert(formData);
	console.log(formData);
	//console.log(save_method);
	$.ajax({
		url : "{{ URL::to('/admin/payroll/importExcel') }}",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
		...
		success: function(data)
		{
			...
			if(data.status) 
			{
				...
				$("#form_bulk")[0].reset();
				$('#quest_import').html('<span style="color:green;margin-left:20px;font-size:20px;">Questions Imported Succesfully</span>');
				//reload_table();
			}
			 else
			{
		...
		error: function (data)
		{
			alert('Please Upload Question Excel Sheet');
			$('#btnSave').text('save'); //change button text
			$('#btnSave').attr('disabled',false);  //set button enable  
		}
	});
